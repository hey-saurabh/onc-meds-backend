...
class DB():
    ...
    async def create(cls, data={}):
        try:
            if data:
                data = timestamp(data)
                db_document = database[cls.__name__.lower()].insert_one(data)
    ...
    async def filter(cls, query={}, values=[], order_by=None, paginate=(0,0)):
        try:
            ...
            start, end = paginate
            db_document = database[cls.__name__.lower()].find(clean(query), create_values(values)).skip(start).limit(end).sort(_order[0], _order[1])
